...
describe('Testing roshan state parsing', () => {
  ...
  test('New state', async () => {
    const events = await process(clientId, {
      map: { roshan_state: 'alive', roshan_state_end_seconds: 0 },
    } as Partial<GSIData>);
...
test('Roshan lifecycle', async () => {
  ...
  /** Roshan kill **/
  const roshanKillEvents = await process(clientId, {
    map: { roshan_state: 'respawn_base', roshan_state_end_seconds: 400 },
  } as Partial<GSIData>);
  ...
  const aegisPickupEvents = await process(clientId, {
    map: { roshan_state: 'respawn_base', roshan_state_end_seconds: 390 },
  } as Partial<GSIData>);
  ...
  const aegisDropEvents = await process(clientId, {
    map: { roshan_state: 'respawn_base', roshan_state_end_seconds: 380 },
  } as Partial<GSIData>);
  ...
  const roshanVariableCountdown = await process(clientId, {
    map: { roshan_state: 'respawn_variable', roshan_state_end_seconds: 120 },
  } as Partial<GSIData>);
  ...
  /** Roshan respawn **/
  await setObj(aegisKey(clientId), 0);
  const respawnEvents = await process(clientId, {
    map: { roshan_state: 'alive', roshan_state_end_seconds: 0 },
  } as Partial<GSIData>);
